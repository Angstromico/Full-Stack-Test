...
export const registerUser = async (req: AuthRequest, res: Response, next: NextFunction): Promise<void> => {
  try {
    ...
    const username = req.body.username || auth0Payload.nickname || email.split('@')[0]

    // Check if user already exists
    const existingUser = await User.findOne({
      $or: [{ auth0Id: auth0Payload.sub }, { email: email.toLowerCase() }, { username: username.toLowerCase() }],
    })

    if (existingUser) {
      res.status(BAD_REQUEST).json({ error: 'User already exists with this email, username, or Auth0 ID' })
      return
    }
...
export const loginUser = async (req: AuthRequest, res: Response, next: NextFunction): Promise<void> => {
  try {
    ...
    if (!user) {
      res.status(NOT_FOUND).json({ error: 'User not found. Please register first.' })
      return
    }
